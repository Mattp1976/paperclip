          ...
          {inspectOpen && (
            <div className="mt-3 space-y-3 rounded-xl border border-border/15 dark:border-border/30 bg-muted/30 dark:bg-muted/10 p-3">
              {sorted.map((r, idx) => {
                ...
                const statusIcon =
                  r.status === "succeeded" ? (
                    <CheckCircle2 className="h-3 w-3 text-sage-ink" />
                  ) : r.status === "failed" ? (
      ...
      <div className="flex items-center justify-between border-t border-border/10 dark:border-border/30 px-5 py-2.5">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs text-muted-foreground/70 hover:text-sage-ink dark:hover:text-green-400 hover:bg-muted/60 transition-colors"
