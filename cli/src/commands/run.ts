...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  ...
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@mattparrytfc/server/src/index.ts")) {
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @mattparrytfc/server package
  try {
    const mod = await import("@mattparrytfc/server");
    return await startServerFromModule(mod, "@mattparrytfc/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@mattparrytfc/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Paperclip server entrypoint.\n` +
          `Tried: ${devEntry}, @mattparrytfc/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
