 ...
 *
 * Bundles all workspace packages (@orqestra/*) into a single file.
...
// to npm and resolved at runtime (e.g. @orqestra/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@orqestra/")) {
      externals.add(name);
    }
  }
