...
```bash
pnpm --filter @orqestra/plugin-file-browser-example build
pnpm orqestra plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm orqestra plugin uninstall paperclip-file-browser-example --force
```
