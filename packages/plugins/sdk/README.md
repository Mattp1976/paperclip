...
- **Worker SDK:** `@orqestra/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@orqestra/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@orqestra/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@orqestra/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@orqestra/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@orqestra/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@orqestra/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@orqestra/plugin-sdk/ui/hooks` | Hooks only |
| `@orqestra/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@orqestra/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@orqestra/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@orqestra/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@orqestra/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@orqestra/plugin-sdk/types` | Worker context and API types (advanced) |
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@orqestra/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@orqestra/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@orqestra/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@orqestra/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@orqestra/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@orqestra/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@orqestra/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@orqestra/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@orqestra/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@orqestra/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@orqestra/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@orqestra/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@orqestra/plugin-sdk/bundlers";
...
```ts
import { startPluginDevServer } from "@orqestra/plugin-sdk/dev-server";
