 ...
 * Paper Trader Agent
 * ==================
 * Executes paper trades based on promoted hypotheses.
 * Runs every 5 minutes alongside the scanner.
 *
 * For each hypothesis in "paper_trading" or "testing" status:
 *   1. Check if entry conditions are met against latest snapshots
 *   2. Open paper position if conditions satisfied
 *   3. Monitor open positions for exit conditions
 *   4. Close positions and record P&L
 */
...
export class PaperTrader {
  ...
  private async checkExits(): Promise<void> {
    const openTrades = await this.db
      .select({
        id: tradingPaperTrades.id,
        hypothesisId: tradingPaperTrades.hypothesisId,
        assetId: tradingPaperTrades.assetId,
        direction: tradingPaperTrades.direction,
        entryPrice: tradingPaperTrades.entryPrice,
        entryTime: tradingPaperTrades.entryTime,
    ...
    for (const trade of openTrades) {
      try {
        const [hyp] = await this.db
          ...
          .where(eq(tradingHypotheses.id, trade.hypothesisId))
          .limit(1);
        if (!hyp) continue;
        ...
        if (exitReason) {
          ...
          const [asset] = await this.db
            ...
            .from(tradingAssets)
            .where(eq(tradingAssets.id, trade.assetId));
  ...
  private async checkEntries(): Promise<void> {
    const hypotheses = await this.db
      ...
      .from(tradingHypotheses)
      .where(inArray(tradingHypotheses.status, ["paper_trading", "testing"]));
      ...
      try {
        ...
        for (const symbol of symbols) {
          const [asset] = await this.db
            ...
            .from(tradingAssets)
            .where(eq(tradingAssets.symbol, symbol))
            .limit(1);
          if (!asset) continue;
          ...
          const [snap] = await this.db
            .select({
              ...
              priceChange1h: tradingSnapshots.priceChange1h,
          ...
          if (this.checkConditions(snap, entryRules)) {
            ...
            const qty = (STARTING_CAPITAL * positionPct / 100) / price;

            await this.db.insert(tradingPaperTrades).values({
              hypothesisId: hyp.id,
              assetId: asset.id,
              direction: entryRules.direction,
            ...
            console.log(`[PaperTrader] OPENED ${entryRules.direction.toUpperCase()} ${symbol} @ $${price.toFixed(2)} - ${hyp.name}`);
