...
```bash
pnpm orqestra heartbeat run --agent-id "$PAPERCLIP_AGENT_ID"
```
...
```bash
pnpm orqestra issue update <issue-id> --assignee-agent-id <other-agent-id> --status todo
```
