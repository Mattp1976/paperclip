...
```bash
pnpm --filter @mattparrytfc/create-paperclip-plugin build
...
Inside this monorepo, the scaffold uses `workspace:*` for `@mattparrytfc/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@mattparrytfc/plugin-sdk` from the local Paperclip checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@mattparrytfc/plugin-sdk/ui`
