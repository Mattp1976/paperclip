...
- `pnpm orqestra plugin list`
- `pnpm orqestra plugin install <package[@version]>`
- `pnpm orqestra plugin uninstall <plugin-id>`
- `pnpm orqestra plugin upgrade <plugin-id> [version]`
- `pnpm orqestra plugin doctor <plugin-id>`
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@orqestra/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@orqestra/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@orqestra/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@orqestra/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@orqestra/plugin-sdk/ui`
...
The `@orqestra/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
6. The operator may force-purge immediately via CLI: `pnpm orqestra plugin purge <plugin-id>`.
...
### 27.1 `@orqestra/plugin-test-harness`
...
```ts
import { createTestHarness } from "@orqestra/plugin-test-harness";
...
- The operator installs the plugin from a local path: `pnpm orqestra plugin install ./path/to/plugin`
...
- `@orqestra/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@orqestra/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@orqestra/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@orqestra/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@orqestra/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@orqestra/plugin-sdk/ui`
