...
- **`id`**: Unique identifier, used as the npm package name suffix (`@orqestra/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@orqestra/core";
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@orqestra/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@orqestra/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
...
```bash
pnpm orqestra store list                    # browse available modules and templates
pnpm orqestra store install <module-id>     # install a module
pnpm orqestra store import <template-id>    # import a company template
pnpm orqestra store export                  # export current company as template
```
...
10. **`@orqestra/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
