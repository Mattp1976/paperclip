...
- local mode that still feels instant (`npx orqestra run` and go)
...
2. CLI command `pnpm orqestra auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm orqestra onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm orqestra onboard`).
...
- expose key instance settings in API + CLI (`orqestra configure` / `orqestra onboard`)
...
7. `pnpm orqestra onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
