...
- `orqestra onboard` writes a valid `storage` config block by default.
- `orqestra configure --section storage` can switch between local and s3 modes.
...
- `orqestra doctor` reports actionable storage status.
