...
## 2. Shared Types (`@orqestra/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@orqestra/adapter-utils` (types) or `@orqestra/adapter-utils/server-utils` (runtime helpers).
...
```json
{
  ...
  "dependencies": {
    "@orqestra/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@orqestra/adapter-utils/server-utils`)
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@orqestra/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@orqestra/adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@orqestra/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@orqestra/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@orqestra/adapter-my-agent/cli";
...
Import from `@orqestra/adapter-utils/server-utils`:
...
- Package name: `@orqestra/adapter-<kebab-name>`
