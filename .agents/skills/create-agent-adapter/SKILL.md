...
## 2. Shared Types (`@mattparrytfc/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@mattparrytfc/adapter-utils` (types) or `@mattparrytfc/adapter-utils/server-utils` (runtime helpers).
...
```json
{
  "name": "@mattparrytfc/adapter-<name>",
  "version": "0.0.1",
  "private": true,
  "type": "module",
  "exports": {
  ...
  "dependencies": {
    "@mattparrytfc/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@mattparrytfc/adapter-utils/server-utils`)
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@mattparrytfc/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@mattparrytfc/adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@mattparrytfc/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@mattparrytfc/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@mattparrytfc/adapter-my-agent/cli";
...
Import from `@mattparrytfc/adapter-utils/server-utils`:
...
- Package name: `@mattparrytfc/adapter-<kebab-name>`
